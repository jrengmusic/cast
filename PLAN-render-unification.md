...
**Date:** 2026-08-22 (revision 6 — revision 5 plus session rulings 26-35; chat rulings are ground truth, this document mirrors them)
...
| 15 | ~~Bimap key type is parametrized, one block~~ — **superseded by 27** (specialized blocks: `bimap` int-baked + `bimapUint32`) |
...
| 25 | ~~Uniform LUT brief line~~ — **superseded by 29** (comment vocabulary abolished) |
| 26 | **Grid tables everywhere (ARCHITECT):** pipe emission dead — one grammar, pandoc grid. Canon: single-line rows stack with NO borders between them; borders enclose multi-line rows; column widths = content max; trimmed cells are Document state, layout derived on write — the layout IS the state. Writer border law: interior border only between multi-line rows (jam_MarkdownWriter.h `isMultiLine`); parser continuation law: an interior line whose first cell starts with a bullet or quote marker continues the current row (`quoteMarker` mirrors `bulletMarker`, jam_MarkdownDocument.h:2415-2428). Both names session-introduced |
| 27 | **Specialized template blocks (ARCHITECT):** types and capacity are baked into per-specialization blocks — table rows carry NO type/capacity/keyType/valueType parameters and NO code fragments; a row only refers `template:<id>`. Blocks: 23 `lookupTable*` (one per type2/capacity tuple), `hashMapString`/`hashMapInt`, `bimap` (int) + `bimapUint32` (ColourNames). Generic `lookupTable`/`hashMap` blocks and all manifest type/capacity aliases (`@uint32 @id @string @cString @wchar @cssCapacity @htmlBlockCapacity @htmlType1Capacity`) deleted from CAST.md; `@id`/`@string` live on ONLY in the per-file `## index` of lexicon.md/files.md (decision 24 unchanged) |
| 28 | **Default is always first row — ALL map types (ARCHITECT):** universal law for Bimap, LookupTable, HashMap, and any future map: the source table's first row IS the default; no default machinery anywhere — no `- value:`/`- default:` bindings, no default codeblocks, no explicit-default constructor emission. LookupTable: single-arg blocks, `defaultValue` derived from `entries[0].value` (jam_LookupTable.h:82-100). Bimap: base `getDefault()` already returns the first authored entry's value (jam_Bimap.h:184-187) — generated structs emit NO getDefault override; the bimap blocks carry none. Any map type whose framework class lacks first-row default derivation gets it there — the law lives in the type, never in per-table data. Convergence diffs in getDefault class are oracle re-promotion, ARCHITECT-reviewed |
| 29 | **Comment vocabulary abolished (ARCHITECT):** no `- comment:` bindings, no `:::comment:*:::` lines in blocks, no doc-brief emission. Unmatched placeholder = trim (marker + one preceding whitespace), same elision as empty value; no fallbacks, no hardcoded placeholder names — pure token match |
| 30 | **Generated reversal (ARCHITECT):** jam_Generated.h emits `namespace map { struct Generated { jam::SharedInstance<X> xCamel { std::in_place }; } }` — the `generated` block is literal |
| 31 | **Mermaid prefix dropped (ARCHITECT):** the 19 mermaid.md table headings are un-prefixed (`## nodeShapes` …); `@mermaid:<table>` refs match; generated `- name:` symbols keep the `mermaid` prefix. Amends the rev-jam mermaid sandbox: mermaid.md edits are ARCHITECT-directed this session |
| 32 | **Bimap construct = one bordered row (ARCHITECT):** dual feeds (`> > > - line:` mapEntry, `> > - line:` enum) are two placeholder-cell lines of ONE multi-line row — the 96 split blocks merged (241 → 145 blocks); Validator's binding-only `continue` concession removed — depth-0 row without a template reference fails |
| 33 | **No alias-into-parameter (ARCHITECT):** code fragments never appear in tables — every fragment is a template codeblock referenced `template:<id>`; index aliases carry symbols/paths only |
| 34 | **NO SANDBOX NO BUILD (ARCHITECT):** agents never build, never run the binary, never run git (read-only included). ARCHITECT builds and tests; agents read files and runtime evidence ARCHITECT produces |
| 35 | **Chat rulings are ground truth:** an ARCHITECT ruling in session supersedes this document immediately; the document is updated to mirror it in the same session |
...
**Status 2026-08-22 (rev 6):** Steps 2-4 landed per decisions 26-33 (grid-everywhere writer + parser continuation, specialized blocks, defaults-as-first-row, bimap merge, mermaid un-prefix, comment abolition) — ALL UNVERIFIED: not built, not run since the rewrite. Step 6 is next: expected diff classes are getDefault (decision 28) and brief-line removal (decision 29); every other hunk is a discrepancy root-caused at its owner.
