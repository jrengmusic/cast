...
private:
     ...
     */
    static juce::String getText (const jam::Array<Element*>& children);
    ...
    /** Highest codepoint value an editor's \<xx\> placeholder covers before widening to \<xxxx\>. */
    static constexpr uint32_t unprintableByteMax { 0xff };

    /** Cells an editor's \<xx\> placeholder occupies for an unprintable codepoint at or below U+00FF. */
    static constexpr int unprintableByteCells { 4 };
    /** Cells an editor's \<xxxx\> placeholder occupies for an unprintable codepoint above U+00FF. */
    static constexpr int unprintableCodepointCells { 6 };

    /**
     * @brief The cell width a grapheme cluster's head codepoint contributes to a rendered column.
     *
     * A codepoint with a positive AttributedChar::width() contributes that
     * width directly (1 for regular, 2 for wide). A negative width falls
     * back to one cell. A codepoint with zero width is either a combining
     * mark — a zero-advance extender folded into its base cluster, still
     * counted as one cell here since a cluster's own head supplies the
     * remaining width — or an invisible codepoint rendered as an editor's
     * placeholder, whose placeholder width is unprintableByteCells at or
     * below unprintableByteMax and unprintableCodepointCells above it.
     *
     * @param codepoint The grapheme cluster's head codepoint.
     * @return The number of cells the cluster's head codepoint occupies.
     */
    static int getClusterHeadCells (uint32_t codepoint) noexcept;

    /**
     * @brief The rendered display width of text, measured in grapheme clusters rather than codepoints.
     *
     * Walks text one codepoint at a time through
     * AttributedChar::graphemeSegmentationStep(); a codepoint that extends
     * the current cluster (segmentation.addToCurrentCell()) adds no further
     * width, and a codepoint that starts a new cluster adds
     * getClusterHeadCells() for its own head codepoint.
     *
     * @param text The text to measure.
     * @return The text's rendered display width, in cells.
     */
    static int getDisplayWidth (const juce::String& text) noexcept;
     ...
     */
    static juce::String getParagraphText (const Element& block);
     ...
     */
    static juce::String getBlockquoteText (const Element& block);
     ...
     */
    static juce::String getListText (const Element& block);
     ...
     */
    static jam::Strings getListItemLines (const Element& block);
     ...
     */
    static juce::String getListItemText (const Element& block);
     ...
     */
    static jam::Array<int> getTableColumnWidths (const Element& block,
                                                 const jam::Array<juce::String>& alignment);
     ...
     */
    static juce::String getGridTableText (const Element& block, const Element& header,
                                          const jam::Array<int>& columnWidths,
                                          const jam::Array<juce::String>& alignment);
     ...
     */
    static juce::String getTableText (const Element& block);
     ...
     */
    static juce::String getText (const Element& element);
};
