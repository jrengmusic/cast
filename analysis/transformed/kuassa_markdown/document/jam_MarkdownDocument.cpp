...
MarkdownDocument MarkdownDocument::parse (const juce::String& documentText)
{
    ...
    document.Document::parse (
        documentText.toRawUTF8(), static_cast<int> (documentText.getNumBytesAsUTF8()));
    return document;
}
...
jam::Array<Document::Element*> MarkdownDocument::getTables() const
{
    jam::Array<Element*> result;

    for (auto* child : *root)
        if (child->contains (Id::type) and *child->get<int> (Id::type) == map::BlockType::table)
            result.add (child);

    return result;
}

jam::Array<Document::Element*> MarkdownDocument::getTables (const juce::Identifier& tableId) const
{
    jam::Array<Element*> result;
...
jam::Array<Document::Element*> MarkdownDocument::getTableRows (Element& table) const
{
    jam::Array<Element*> result;
...
jam::Array<juce::String> MarkdownDocument::getTableHeaders (Element& table) const
{
...
Document::Element* MarkdownDocument::getTableRow (Element& table, const juce::Identifier& rowId) const
{
    ...
    const auto rowIndex { elements.find (probe) };

    if (rowIndex >= 0)
        result = elements.at (static_cast<size_t> (rowIndex)).get();

    return result;
}

Document::Element* MarkdownDocument::getTableCell (Element& table,
...
juce::String MarkdownDocument::getTableValue (Element& table,
                                              const juce::Identifier& colId,
                                              const juce::Identifier& rowId) const
{
    juce::String result;
...
Document::Element* MarkdownDocument::getTableCell (Element& row, const juce::Identifier& colId) const
{
    Element* result { nullptr };
    ...
    const auto cellIndex { elements.find (probe) };

    if (cellIndex >= 0)
        result = elements.at (static_cast<size_t> (cellIndex)).get();

    return result;
}

juce::String MarkdownDocument::getTableValue (Element& row, const juce::Identifier& colId) const
{
    juce::String result;
...
std::string_view MarkdownDocument::getTableValueView (Element& row, const juce::Identifier& colId) const
{
...
Document::Element* MarkdownDocument::getTableList (Element& row, const juce::Identifier& colId) const
{
    Element* result { nullptr };
...
std::string_view MarkdownDocument::getTableValueView (Element& row,
                                                       const juce::Identifier& colId,
...
bool MarkdownDocument::hasTableValue (Element& row, const juce::Identifier& colId) const noexcept
{
...
jam::Array<Document::Element*> MarkdownDocument::getTableRows (const juce::Identifier& tableId) const
{
    jam::Array<Element*> result;
...
Document::Element* MarkdownDocument::getTableCell (const juce::Identifier& tableId,
                                                   const juce::Identifier& colId,
                                                   const juce::Identifier& rowId) const
{
    Element* result { nullptr };
...
Document::Element* MarkdownDocument::getTableRow (const juce::Identifier& tableId, const juce::Identifier& rowId) const
{
    Element* result { nullptr };
...
Document::Element* MarkdownDocument::getCodeBlock (const juce::Identifier& codeId) const noexcept
{
    Element probe;
    probe.parent = root;
    probe.id = codeId;
    ...
    Element* result { nullptr };
    const auto index { elements.find (probe) };

    if (index >= 0)
        result = elements.at (static_cast<size_t> (index)).get();

    return result;
}

Document::Element* MarkdownDocument::getBlockquote (Element& scope) const noexcept
{
    Element probe;
    probe.parent = &scope;
    probe.id = Id::blockquote;
...
Document::Element* MarkdownDocument::getList (Element& scope) const noexcept
{
    Element probe;
    probe.parent = &scope;
    probe.id = Id::ul;
...
Document::Element* MarkdownDocument::getListItem (Element& list, const juce::Identifier& itemId) const noexcept
{
    Element probe;
    probe.parent = &list;
    probe.id = itemId;
...
Document::Element* MarkdownDocument::getTable (const juce::Identifier& tableId) const noexcept
{
    Element probe;
    probe.parent = root;
    probe.id = tableId;
...
Document::Element* MarkdownDocument::getTableHeaderRow (const Element& table) noexcept
{
    for (auto* row : table)
