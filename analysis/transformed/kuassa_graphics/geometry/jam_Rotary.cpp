                                           ...
                                           float rotation)
{
    ...
    for (int index { 0 }; index < numbers.size(); ++index)
    {
        ...
        if (bool isShow { static_cast<bool> (nsv[isShowing].getIntValue()) })
        {
            ...
            auto line { jam::centredLine (area.getCentre(), inRadius, outRadius, angles[index]) };
...
void Rotary::Line::draw (juce::Graphics& g)
{
    ...
    switch (drawArc)
    {
        ...
        default:
            ...
            {
                path.addLineSegment (jam::centredLine (centre, radius, length, angle), 0.0f);
            }
            break;
    }
...
void Rotary::LineSub::draw (juce::Graphics& g)
{
    ...
    {
        if (arcPosition)
            subPath.addLineSegment (jam::centredLine (centre, radius, radius + subLength, angle), 0.0f);
        else
            subPath.addLineSegment (jam::centredLine (centre, length - subLength, length, angle), 0.0f);
    }
