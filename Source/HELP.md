...
*   `@cell@` — the current row's value in the patch cable's filter column (per-row; transforms apply, e.g. `@cell:toPascal@`). Available in both row-region and per-row fragment expansion.
...
*   `@column@` — the patch cable's filter column name (transforms apply, e.g. `@column:toPascal@`). Available in both row-region and per-row fragment expansion.
...
*   `## patch` — Maps a **patch cable**: `source | fragment | placeholder`. One cable, one row — every cable states all three cells; no cell inherits from the row above.
    *   **Source** resolves one of three ways. A bare **table** name resolves in the tables namespace (the spliced `tables/*.md` domain tables), and its rows match through that table's own column 0. A **dot-address** — `table.column` — splits on the first `.`: the left side names the table, the right side names the filter column (`@column@`/`@cell@`) in place of column 0. A **bus** — a source that names another cable's `placeholder` — routes everything already patched into that placeholder through this cable's own placeholder: a bus cable's placeholder receives the matched placeholder's expanded results appended in cable order. A bus cable with an empty `fragment` cell is pure routing — it expands nothing of its own.
...
Every generated construct is plain text substitution — nothing more. These examples show each pairing end to end. In all of them the patch cable's `source` names the table (a bare table name, or a `table.column` dot-address), and the `placeholder` cell names the root template's aggregate region that collects the expanded fragments.
