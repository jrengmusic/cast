...
bool MarkdownDocument::BlockParser::isSetextUnderline (const std::string& source, size_t start, size_t end) noexcept
{
    return getSetextLevel (source, start, end) != 0;
}
...
bool MarkdownDocument::BlockParser::isThematicBreak (const std::string& source, size_t start, size_t end) noexcept
{
    return hasUnindentedStart (source, start, end)
           and isThematicBreakContent (source, trimStart (source, start, end), end);
}

bool MarkdownDocument::BlockParser::addThematicBreak (size_t start, size_t end, size_t& remainderStart)
{
    const auto matched { isThematicBreak (document.getSource(), start, end) };
...
bool MarkdownDocument::BlockParser::isAtxHeading (const std::string& source, size_t start, size_t end) noexcept
{
    return hasUnindentedStart (source, start, end)
           and getAtxHeadingLevel (source, trimStart (source, start, end), end) > 0;
}
...
bool MarkdownDocument::BlockParser::addAtxHeading (size_t start, size_t end, size_t& remainderStart)
{
    ...
    if (not isAtxHeading (source, start, end))
        return false;

    const auto trimmedStart { trimStart (source, start, end) };
    const auto level { getAtxHeadingLevel (source, trimmedStart, end) };
...
bool MarkdownDocument::BlockParser::isBlockquote (const std::string& source, size_t start, size_t end) noexcept
{
    return getBlockquoteMarker (source, start, end) > 0;
}

bool MarkdownDocument::BlockParser::addBlockquote (size_t start, size_t end, size_t& remainderStart)
{
    const auto& source { document.getSource() };
    const auto matched { isBlockquote (source, start, end) };
    ...
    if (matched)
    {
        ...
        containerDepth += 1;
        remainderStart = advance (start, end, getBlockquoteMarker (source, start, end));
    }
...
bool MarkdownDocument::BlockParser::isListItem (const std::string& source, size_t start, size_t end,
                                                bool interruptingParagraph)
{
    ...
    return getListItemIndentWidth (source, afterMarkerStart, end, markerWidth) != 0;
}
...
MarkdownDocument::BlockParser::addListItem (size_t start, size_t end, size_t& remainderStart, bool allContainersMatched)
{
    ...
    if (not isListItem (source, start, end, interruptingParagraph))
        return false;
    ...
    const auto afterMarkerStart { advance (contentStart, end, markerWidth) };
    const auto indentWidth { getListItemIndentWidth (source, afterMarkerStart, end, markerWidth) };
...
bool MarkdownDocument::BlockParser::isFencedCode (const std::string& source, size_t start, size_t end)
{
    return getFenceOpenWidth (source, start, end) > 0;
}
...
bool MarkdownDocument::BlockParser::addFencedCodeBlock (size_t start, size_t end)
{
    ...
    if (not isFencedCode (source, start, end))
        return false;
...
const jam::Function::Map<int, bool>& MarkdownDocument::BlockParser::getHtmlBlockConditions()
{
    static const auto htmlBlockConditions {
        []
        {
            jam::Function::Map<int, bool> htmlBlockConditions;

            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::rawText, isHtmlBlockType1Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::comment, isHtmlBlockType2Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::processingInstruction, isHtmlBlockType3Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::declaration, isHtmlBlockType4Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::cdata, isHtmlBlockType5Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::blockTag, isHtmlBlockType6Start);
            htmlBlockConditions.add<const std::string&, size_t&, size_t&> (map::HtmlBlockType::anyTag, isHtmlBlockType7Start);

            return htmlBlockConditions;
        }()
    };
...
int MarkdownDocument::BlockParser::getHtmlBlockCondition (const std::string& source, size_t start, size_t end)
{
    ...
    const auto& htmlBlockConditions { getHtmlBlockConditions() };

    for (int type { map::HtmlBlockType::rawText }; type <= map::HtmlBlockType::anyTag; ++type)
        if (htmlBlockConditions.get (type, source, start, end))
            return type;
...
bool MarkdownDocument::BlockParser::isHtmlBlock (const std::string& source, size_t start, size_t end,
                                                 bool interruptingParagraph)
{
    if (not hasUnindentedStart (source, start, end))
        return false;

    const auto conditionType { getHtmlBlockCondition (source, trimStart (source, start, end), end) };

    return conditionType != map::HtmlBlockType::none
           and not (conditionType == map::HtmlBlockType::anyTag and interruptingParagraph);
}
...
bool MarkdownDocument::BlockParser::addHtmlBlock (size_t start, size_t end, size_t& remainderStart)
{
    ...
    if (not isHtmlBlock (source, start, end, interruptingParagraph))
        return false;
...
Document::Element* MarkdownDocument::BlockParser::replaceLastChild (Element& parent, const juce::Identifier& id)
{
    ...
    document.removeChild (parent, *replaced);
...
bool MarkdownDocument::BlockParser::isTable (const std::string& source, size_t start, size_t end) noexcept
{
    return not getRowAlignments (source, start, end).isEmpty();
}
...
bool MarkdownDocument::BlockParser::addTable (size_t start, size_t end)
{
    ...
    if (not isTable (document.getSource(), start, end))
        return false;
...
void MarkdownDocument::BlockParser::applyContinuationResult (int matchedDepth, size_t start, size_t end)
{
    if (matchedDepth < containerDepth)
    {
        ...
        const auto beginsBlock {
            isThematicBreak (source, start, end)
            or isAtxHeading (source, start, end)
            or isBlockquote (source, start, end)
            or isFencedCode (source, start, end)
