...
@brief DEC private mode numbers used in CSI ? Ps h / CSI ? Ps l (DECSET/DECRST), DECRQM (CSI ? Ps $
p), and DECRQSS (CSI Ps $ q).
...
@brief CSI intermediate bytes — the bytes preceding the final that select a private or otherwise-
disambiguated command variant.
...
@brief Progressive keyboard protocol assignment sub-parameter values — the mode in CSI = flags ;
mode u.
