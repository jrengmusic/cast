...
```
@brief ANSI terminal token kinds — text run, CSI sequence, OSC sequence, and control.

Classifies each token the ANSI tokeniser emits. `text` is a printable run;
`csi` is a Control Sequence Introducer sequence accumulated to its final
byte; `osc` is an Operating System Command sequence run to its terminator;
`control` is a C0 single (LF and the remaining bare control bytes). The key
is the discriminator the parser dispatches on when building the cell tree.
```
...
```
@brief Message-overlay split axis line style names.

Consumed by drawMessageOverlay's axisLineStyle key; the default row is
solid.
```
...
```
@brief Text editor modes — normal, insert, visual.
```
...
```
@brief Text editor state properties persisted on the editor's parameter node.
```
...
```
@brief Visual selection shapes — character, line, block.
```
...
```
@brief Unicode Line_Break classes (UAX #14, Unicode 17.0.0).

Classifies each codepoint by its line-breaking behavior. The key is the
class the range-lookup tables store per codepoint, letting the break step
skip re-testing a codepoint's properties on every pass.
```
...
```
@brief Language input for UAX #14 tailoring (ISO 639-1; und = none).
```
